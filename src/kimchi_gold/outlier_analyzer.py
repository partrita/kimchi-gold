...
def calculate_statistical_outlier_boundaries(
    ...
    lower_boundary = first_quartile - interquartile_range_multiplier * interquartile_range
    upper_boundary = third_quartile + interquartile_range_multiplier * interquartile_range
...
def filter_dataframe_by_recent_dates(
    ...
    processed_dataframe[date_column_name] = pd.to_datetime(processed_dataframe[date_column_name])
    ...
    logger.debug(f"필터링 결과: 전체 {len(source_dataframe)} 행 → {len(filtered_dataframe)} 행")
...
def determine_if_latest_value_is_outlier(
    ...
    if len(recent_period_data) < 4:  # 사분위수 계산에 최소 4개 데이터 필요
        logger.warning(f"사분위수 계산에 충분한 데이터가 없습니다 (현재: {len(recent_period_data)}개)")
        return False
    ...
    is_outlier_detected = (most_recent_value < lower_boundary) or (most_recent_value > upper_boundary)
