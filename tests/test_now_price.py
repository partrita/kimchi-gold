...
# Allowed domain for testing
VALID_TEST_URL = "https://m.stock.naver.com/test"
...
def test_get_price_from_naver_success():
    ...
    with (
        ...
        mock_response.raw.read.return_value = f"""
            <html>
                <body>
        ...
        mock_get.assert_called_once_with(
            url, headers=price_fetcher.REQUEST_HEADERS, timeout=10, stream=True
        )
        # BeautifulSoup is called with the content read from response.raw.read
...
def test_url_safety_validation():
    with pytest.raises(ValueError) as excinfo:
        price_fetcher.extract_price_from_naver_finance("https://malicious.com", "Error")
    assert "허용되지 않은 URL입니다" in str(excinfo.value)
